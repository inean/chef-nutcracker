...
instances["instances"].each do |instance|
  ...
  execute "update-rc.d nutcracker_#{instance['port']}" do
    command "sudo update-rc.d nutcracker_#{instance['port']} defaults > /tmp/nutcracker_#{instance['port']}.update-rc.d_log 2>&1 || exit $?"
  ...
  execute "start nutcracker_#{instance['port']}" do
    command "sudo /etc/init.d/nutcracker_#{instance['port']} start > /tmp/nutcracker_#{instance['port']}.startup_log 2>&1 || exit $?"
